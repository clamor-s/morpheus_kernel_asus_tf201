...
static int third = 342000;
static int fourth = 427000;
static int fifth = 475000;

static int lp_core_clocking(void)
{
	/*
	 * Max low power core clock should be set exactly
	 * to 555, 620, 666, 700 or 740 mHz.
	 * If clock is set in between this values it must
	 * be corrected to the closest value.
	 */
	...
	if (lp_clock <= 555000) {
		...
		fourth = 427000
		fifth = 500000
		return lp_clock = 555000;
	}

	if (lp_clock > 555000 && lp_clock <= 620000) {
		third = 342000
		fourth = 427000
		fifth = 500000
		return lp_clock = 620000;
	}

	if (lp_clock > 620000 && lp_clock <= 666000) {
		third = 342000
		fourth = 475000
		fifth = 555000
		return lp_clock = 666000;
	}

	if (lp_clock > 666000 && lp_clock <= 700000) {
		third = 427000
		fourth = 555000
		fifth = 620000
		return lp_clock = 700000;
	}
	...
	if (lp_clock > 700000) {
		...
		fourth = 555000
		fifth = 666000
		return lp_clock = 740000;
	}
}
#endif

#ifdef CONFIG_GPU_OVERCLOCK
int gpu_clock = 416000;
module_param(gpu_clock, int, 0644);
MODULE_PARM_DESC(gpu_clock, "GPU over-clocking");
...
static int gpu_core_clocking(void)
{
	/*
	 * Max low power core clock should be set exactly
	 * to 416, 446, 484, 520, 600, 666 or 700 mHz.
	 * If clock is set in between this values it must
	 * be corrected to the closest value.
	 */
	...
	if (gpu_clock <= 416000) {
		...
		fourth = 427000
		fifth = 500000
		return gpu_clock = 416000;
	}

	if (gpu_clock > 416000 && gpu_clock <= 446000) {
		third = 342000
		fourth = 427000
		fifth = 500000
		return gpu_clock = 446000;
	}

	if (gpu_clock > 446000 && gpu_clock <= 484000) {
		third = 342000
		fourth = 475000
		fifth = 555000
		return gpu_clock = 484000;
	}

	if (gpu_clock > 484000 && gpu_clock <= 520000) {
		third = 427000
		fourth = 555000
		fifth = 620000
		return gpu_clock = 520000;
	}

	if (gpu_clock > 520000 && gpu_clock <= 600000) {
		third = 427000
		fourth = 555000
		fifth = 620000
		return gpu_clock = 600000;
	}

	if (gpu_clock > 600000 && gpu_clock <= 666000) {
		third = 427000
		fourth = 555000
		fifth = 620000
		return gpu_clock = 666000;
	}
...
#ifdef CONFIG_LP_OVERCLOCK
	CORE_DVFS("cpu_lp", 1, 1, KHZ,   204000, 294000, third, fourth,  fifth, lp_clock, lp_clock, lp_clock, lp_clock),
#else
...
#ifdef CONFIG_GPU_OVERCLOCK
	CORE_DVFS("vde",    1, 1, KHZ,   200000, 228000, 275000, 332000, five_g, gpu_clock, gpu_clock, gpu_clock, gpu_clock),
	CORE_DVFS("mpe",    1, 1, KHZ,   200000, 234000, 285000, 332000, five_g, gpu_clock, gpu_clock, gpu_clock, gpu_clock),
	CORE_DVFS("2d",     1, 1, KHZ,   200000, 267000, 285000, 332000, five_g, gpu_clock, gpu_clock, gpu_clock, gpu_clock),
	CORE_DVFS("epp",    1, 1, KHZ,   200000, 267000, 285000, 332000, five_g, gpu_clock, gpu_clock, gpu_clock, gpu_clock),
	CORE_DVFS("3d",     1, 1, KHZ,   200000, 234000, 285000, 332000, five_g, gpu_clock, gpu_clock, gpu_clock, gpu_clock),
	CORE_DVFS("3d2",    1, 1, KHZ,   200000, 234000, 285000, 332000, five_g, gpu_clock, gpu_clock, gpu_clock, gpu_clock),
	CORE_DVFS("se",     1, 1, KHZ,   200000, 267000, 285000, 332000, five_g, gpu_clock, gpu_clock, gpu_clock, gpu_clock),
#else
...
#ifdef CONFIG_GPU_OVERCLOCK
	CORE_DVFS("host1x", 1, 1, KHZ,   100000, 152000, 188000, 222000, pre_gpu, half_gpu, half_gpu, half_gpu, half_gpu),
#else
...
#ifdef CONFIG_GPU_OVERCLOCK
	CORE_DVFS("cbus",   1, 1, KHZ,   200000, 228000, 275000, 332000, 380000, gpu_clock, gpu_clock, gpu_clock, gpu_clock),
#else
...
#ifdef CONFIG_GPU_OVERCLOCK
	CORE_DVFS("pll_c",  -1, 1, KHZ,  533000, 667000, 667000, 800000, 800000, two_gpu, two_gpu, two_gpu, two_gpu),
#else
